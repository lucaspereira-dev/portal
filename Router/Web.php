...
class Web{
    ...
    private $method, $path, $params = Array(); // Metodo, URL e parametros da requisição atual
    ...
    private function checkUrl(string $route, $path)
    {
        ...
        // Comentário das impressão do teste de condições
        // var_dump($keys[0]);
        // echo "=>";
        // var_dump($values);
        // echo "<br><br> Count: ". count($keys[0]) ." | ". count($values);
        // echo "<hr>";

        if((count($keys[0]) === count($values)) && (count($keys[0]) >= 1 && count($values) >= 1)){
            array_push($this->params, array_combine($keys[0], $values));
        }
